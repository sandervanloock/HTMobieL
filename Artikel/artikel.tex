...
\begin{equation}
  SU_f = \sum_{c=1}^{8}{\left(\sum_{i=1}^{N}C_{f,c,i}\right)}
...
% \begin{multline}
%   PERF_f = \sum_{c=1}^{8}{\left(l_{f,c,POC}+l_{f,c,POC_{cache}}+l_{f,c,login} \\
% 	 + l_{f,c,login_{cache + l_{r,c,list}}}\right)} 
%   \label{eq:performantie}
% \end{multline}

\begin{equation}
\begin{split}
PERF_f =  \sum_{c=1}^{8}\left(l_{f,c,POC}+l_{f,c,POC_{cache}} \right.\\  
       \left. + l_{f,c,login} + l_{f,c,login_{cache}} + l_{r,c,list} \right)
\end{split}
\end{equation}
...
This section introduces the compared mobile HTML5 frameworks: \st{}~(\sta{}), \kendo{}~(\kendob{}), \jqm{}~(\jqma{}) and \lungo{}.
...
\jqma{} is a mobile HTML5 user interface~(UI) framework that was announced in 2010~\cite{Resig2010}. 
