...
Initially, render times would be measured by the time to render the $850$ list items.
...
Frameworks can be categorised in two approaches: markup driven and \js{} driven~\cite{Oeflman2011}.
...
Ext JS is a JavaScript framework for the development of desktop applications.  
...
\sta{} is written on top of Ext JS and is a JavaScript driven framework.  
...
Although most mobile browsers contain this engine,  some do not like Internet Explorer Mobile.  
...
One can obtain these components by writing HTML5 with \jqma{} specific data attributes. 
...
\paragraph{Code and development}
Programming a \lungo{} application is done by annotating the HTML code with CSS classes and data attributes.
...
To fetch the PDF however in \chal{pdf}, most frameworks have trouble if this is done via an AJAX request because AJAX is not the preferred way for fetching raw data.
...
\begin{figure}[t]
  ...
  \subfloat[Login application]{
    \includegraphics[width=\columnwidth]{../Masterproef/figuren/performantie-login-en.pdf}
    \label{fig:performantie-login}
  }
  \quad
  \subfloat[Login application from cache]{
    \includegraphics[width=\columnwidth]{../Masterproef/figuren/performantie-login-cache-en.pdf}
