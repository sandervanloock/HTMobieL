%TODO challenges uitleggen
%TODO devices introduceren? ondersteuning blijft vaag
...
Firstly priorities are given to the criteria and secondly the candidates are compared pairwise.
...
Our method lets researchers evaluate frameworks without taking the other frameworks into calculation. 
%TODO is dit ook nodig in paper?
%Een laatste opmerking is bij AHP gewichten aan de criteria worden gegeven.
%De voorgestelde methode stelt ieder criterium gelijk, maar het is echter mogelijk om hier van af te wijken.
...
This representation similar to a blogpost that compared native versus web applications~\cite{Jeroen2012}.   

\paragraph{Popularity}
Social networks provide a good indication to determine the popularity that goes with the framework.  
It is also an indication of the future the framework has~\cite{Sarrafi2012a}.
...
Furthermore, the ISO 25010 standard uses usability the check the efficiency of reaching goals with software.
...
Only the challenges where the framework actually provided an implementation ($C_{r,i} > 0$) are checked for support.
...
The result will be a total ordening of the frameworks per context that indicate the smoothness.
...
Initially, render times would have measure the time to render the $850$ list items.
This would be added to the download time to get the total performance.
...
Frameworks can be categorised in two approaches: mark-up driven and \js{} driven~\cite{Oeflman2011}.
...
The first stable version of \sta{} was officially released in November 2010.
...
A useful tool to discover the \sta{} features is the Kitchen Sink~\cite{Inc.2013}.  
...
\lungo{}~(\lungoa) is a markup-driven framework of which version~1.0 appeared in 2011~\cite{TapQuo2011}.
...
\caption{Popularity on May 8, 2013.}
...
This determine the score for this criterion.
...
Secondly,  boilarplate code should be used to initialize a new project.
...
No framework has a documentaion that excels in all three components.  
...
Remarkable is the maximum score of \kendoa{} for \chal{formulieren} and the lower score for \chal{offline} in comparison with \sta.
...
Furthermore \lungo{} gets a zero for \chal{validatie} and also has problems with the more advanced form elements in \chal{formulieren}.
One has to have the luck of finding a plugin or implement the functionality by hand.

It can be generally stated that each of the four frameworks have great support for\chal{laadscherm}.
...
The formula for this criterion can be found in equation~\ref{eq:ondersteuning} were the summation of the support on all devices is explained.
...
The four implementations of this challenge all fail for the \htc{}, \gtab{} and \ipadi{} because the lack of support for one or more of these dependencies~\cite{Deveria2013c,ViljamiSalminen2012}.
...
This is supported for each device~\cite{Deveria2013c}.
...
Finally, offline capabilities are also supported on all eight devices~\cite{Deveria2013c} so each framework achieved the maximum score.
...
\lungo{} has the fastest downloadtime.
\jqm{},  \kendoa{} en \sta{} have respectively the second, third and fourth fastest downloadtime.
\lungo{} is about half of the time faster than the other three frameworks.
The downloadtimes of \kendoa{}, \jqma{} and \lungo{} are proportional to the downloadsize of the framework's resources.
\lungo{} is optimized for mobile development and needs the least resources when looking at \js code.
The \js dependencies of \kendoa{} and \jqma{} are more or less equal in size although the one imposes an architecture and the other does not.
...
When the application is updated,  this mechanism will only download the updated code instead of all the files that are declaired in the manifest file.
...
\jqma{} has the best overal score ($86\%$) and is the winner following our criteria.
...
However,  the advantage in productivity is smaller in comparison with \jqm{} because of the limited documentation.
...
The user experience tests gave an opposite result because after the long download time,  \sta{} had the fastest response times.
...
This will enlarge the comparison itself, but will also keep checking the comparison method.
...
Furthermore the integration of HTML5 features in frameworks likes GPS, push events, drag and drop, video and audio can be researched.

Fourthly, new criteria can be added to the comparison.
It is off interest to incorporate extensibility which states how easy it is to extend an existing application, similar as the portability category of the ISO 25010 standard.
...
A POC was formalised and to drive the usage and device criterion.
...
\jqma{} is highly productive and performant because on the one hand it is well-documented and on the other hand it does not impose an architecture.
...
By contract,  \sta{} scores the best in user experience.
