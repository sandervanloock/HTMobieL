...
%TODO: moeten die 'oude' dingen daar nog inzitten?
Both authors implement the POC two different framework and carefully record the time to implement it.  
...
On the one hand, the average download time of both the non-cached ($l_{f,c,POC_{cache}}$) and cached version ($l_{f,c,login_{cache}}$) will be taken into account.
The formula for average download for a framework $f$ is:

\begin{math}
  \text{Average download time}_r= \frac{\sum\limits_{c=1}^{8}{\left(\widehat{l}_{r,c,login}+\widehat{l}_{r,c,login_{cache}}\right)}}{8}
\end{math}

On the other hand, the user experience by scrolling through a long list of 850 items will be tested.
This is because the calculation of render times with \js{} was not possible because for \sta{} and \lungo{} as they do not have an event after the elements are rendered.
The user experience will be measured on each context with two user tests ($\text{User experience}_{f,c}$).
The result will be a total ordering of the frameworks per context that indicate the responsiveness.
The performance for a framework $f$ is defined by:

\begin{equation}
  \text{Performance}'_r = \frac{\text{Average download time}_r}{\text{Experience}_r}
...
As at the time of writing,  \kendob{} is at version 2013 Q1~\cite{Telerike}. 
...
This is in contrast to \kendob{} and \sta{} that take respectively third  and last place.
...
%TODO is deze figuur nog relevant voor paper?
%\begin{figure}
%  \centering
%  \includegraphics[width=\columnwidth]{../Masterproef/figuren/performance-en.pdf}
...
\lungo{} takes first place and \jqma{} and \kendob{} take the second and third place respectively.
\lungo{} is about have of the time faster than \jqma{}, \kendob{} or \sta{}.
...
When viewing the login application from cache \kendob{}, \jqma{} and \lungo{} have about the same download time.
...
Table \ref{tabel:evaluatie-performantie-gebruikerservaring} shows the the user experience.
\sta{} takes first place with the maximal score.
This means that the scroll experience when best on all the devices.
...
The browser crashed an the device and returned to the home screen.
...
\jqma{} has the largest area ($1.76$) and can be seen as the winner.
...
\kendob{} is second with an area of $1.31$.
The MVVM architecture makes \kendob{} more usable but less productive in comparison with \jqma{}.
However,  the MVVM architecture seems more intuitive than MVC.
The main drawback of \kendob{},  which can not be seen on the figure, is the high license cost.
...
\sta {} is has an area of ($0.75$) and is worst framework following these criteria.
...
As a result,  the downloadtimes were remarkably large.
...
The productivity was measured by time tracking the time to implementing a login application.
...
%TODO dit is veranderd.
Finally,  the load- and response time of the POC and login application determined the performance criterion.
