...
%TODO versies die we nu effectief gebruikt hebben om te implementeren
%ST:  2.1.1
%Kendo: 2013 Q1

%TODO enkel de dingen uitleggen die je hebt gebruikt, dus geen extra info over wat het nog allemaal kan, of dingen die niet in de challenges beschreven staan
...
Een algemene trend bij iedere raamwerk is de volle ondersteuning van AJAX-verzoeken, behalve dan bij één geval voor \lungo{}.
...
Deze maakten namelijk gebruik van \code{li}-items binnen een \code{ul}-tag waardoor er symbool voor de suggesties kwamen.
...
Het offline beschikbaar maken van de applicatie wordt vanuit \kendo{} niet ondersteund.
...
%TODO veel herhaling van tekst want dezelfde aanpak zoals bij kendo (geldt ook voor lungo)
\paragraph{\jqm}
Vanuit het raamwerk komt er standaard geen ondersteuning om gegevens offline te bewaren.
Hiervoor werd gebruik gemaakt van \code{localStorage}, wat gespecificeerd is in HTML5.
Omdat het kan voorkomen dat \code{localStorage} niet wordt ondersteund, werd zelf een \term{fallback} geschreven die de data bijhoudt als \js{}-variabelen.
...
%TODO oorzaak -> gevolg relatie niet correct. Ze verwijzen in jqm documentatie mss naar appcache maar het is niet zo dat de mogelijkheid van appcache de onbeschikbaarheid impliceert
...
Indien het niet ondersteund wordt op het apparaat, zal het raamwerk zelf een \term{fallback} voorzien.
