...
%TODO ik vrees een beetje voor de geldigheid van onderstaande zin
...
%TODO weinig diepgang in deze vier zinnen?
...
%TODO al vroeger vermelden (ook al bij kendo van toepassing)
...
\end{description}
De creatie van de tablet lay-out steunt op de \code{HBox} waarde.
...
Een object van dit xtype kan een bericht bevatten dat de laadtekst voorstelt.
Het tonen van een masker kan door de \code{setMasked} methode op een \code{View} op te roepen.
Een courante aanpak is te werken met de \term{singleton} klasse \code{Ext.Viewport} die de zichtbare \code{View} voorsteld.
...
%TODO einde van de zin:  niet verschijnt verborgen
...
Een \code{Formpanel} van een formulier voorzien kan door in de rij van elementen een object met xtype \code{Fieldset} te maken.
...
Een placeholder toevoegen kan door een veld te voorzien met de \code{placeholder} eigenschap en de waarde aan de gewenste placeholder gelijk te stellen. 
Een label toevoegen verloopt analoog,  deze weglaten zal geen label renderen.

Optievelden toevoegen kan door objecten met het xtype \code{selectfield},  een schakelaar met xtype \code{togglefield}.
...
\paragraph{\kendo}
 ...
 Het \code{placeHolder} attribuut kan een placeholder definiëren,  het vermijden van \code{<label>}-tags zal geen labels genereren.
 ...
 Optievelden toevoegen volgt de HTML-werkwijze.
 Hiervoor moet invoerelementen van het type \code{radio} worden toegevoegd.
 ...
 Een fomulier kan worden gebonden aan een \code{(View) Model}.
 ...
 Wanneer een uitgave wordt toegevoegd, zal de huidige waarde van het \code{(View) Model} wordt het \code{(View) Model} gereset.
...
Velden \term{read-only} maken kan door objecten te voozien van de \code{readOnly} eigenschap en de waarde op \code{true} te zetten.
...
%TODO onderstaande twee zinnen niet super duidelijk
...
Dit bestand moet in de folder touch/src/ux worden geplaatst en de nieuwe klasse moet worden geïnitialiseerd bij het laden van de applicatie.
Hierdoor wordt het xtype \code{autocompletefield} beschikbaar.
...
De \term{backend} vande POC geeft bij een bepaald sleutelwoord suggesties in een JSON-rij terug.
...
Een HTML-invoerelement met \term{identifier} moet worden aangemaakt en geinitialiseerd met \code{\$("\#id").kendoAutoComplete()}.
...
Na het toevoegen van de plug-in wordt het xtype \code{img} beschikbaar voor \st{} componenten.

De plug-in voorziet twee modes voor het opladen van bestanden: lokale als base64 of extern naar een server.
De eerste laat toe afbeeldingen in het DOM of \term{local storage} te laden en wordt door de POC opgelegd.
...
De succes functie krijgt de base64 tekst mee en kan een voorbeeld van de afbeelding weergeven.
...
%TODO streepje tussen HTML5 en File API
...
Deze werkwijze is aanloog aan de \jqm{} en \lungo{} implementatie.
...
Het opbouwen van een foutenboodschap kan door te itereren over de fouten die na validtie worden teruggegeven.
...
De \code{errors} methode van de \code{validator} geeft een rij van foutenboodschappen terug.
...
%TODO naar ondersteuning verplaatsen?
...
%TODO onderstaande twee zinnen spreken elkaar tegen
...
Deze maakt gebruikt van het HTML5-canvas en retourneert de handtekening als base64 tekst.
...
%TODO kunnen direct worden omgegaan
...
%TODO selectoren niet op woordenlijst.org
...
Om een \code{View} te tonen die hoort bij een lijstelement wordt gebruik gemaakt van deze \code{Vavigationview} en de \code{push} methode.
...
Een \code{Controller} kan deze gebeurtenis opvangen en de \code{push} methode op de \code{Vavigationview} oproepen.
