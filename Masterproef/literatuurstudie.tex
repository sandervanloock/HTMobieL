...
Beide verschillen worden gebundeld in de term \term{native look-and-feel} van het mobiel besturingssysteem.
...
Het is net zoals iOS een mobiel besturingssysteem, maar in tegenstelling tot iOS is het \term{open-source}~\cite{David2011}. 
...
Bij beide manieren kan de applicatie net zoals een \term{native} applicatie nog steeds opgeladen te worden naar een winkel.
...
Dit is in tegenstelling tot een \term{native} applicatie die geprogrammeerd wordt aan de hand van een SDK die specifiek is voor het besturingssysteem.
...
Zo kunnen \term{native} applicaties gemakkelijker Central Processing Unit (CPU)-intensieve encryptie uitvoeren en geavanceerde authenticatietechnieken gebruiken zoals gezichtsherkenning.
...
Zoals opgemerkt bij webapplicaties ligt het nadeel bij \term{native} applicaties dat deze worden geprogrammeerd met behulp van een SDK specifiek tot het besturingssysteem.
...
Het voordeel van een hybride applicatie is dat ze de problemen van webapplicaties oplossen met de voordelen van \term{native} applicaties~\cite{Mobile2012}.
...
Mobiele apparaten zijn onstabiel in hun verbinding met het Internet. HTML5 voorziet het offline werken in de cache, lokale opslag (vroeger kon dit enkel via de zogenaamde \term{cookies}) en een API om bestanden te manipuleren.
...
Voor het emuleren van een kenmerk wordt gebruik gemaakt van \term{polyfills}. 
Dit zijn alternatieven op basis van \js{} waarbij de \term{native} functionaliteit die normaal aanwezig moet zijn, geëmuleerd wordt~\cite{Weyl2011}.
...
Daarna zullen \term{fallbacks} geïmplementeerd worden waardoor ook minder recente browsers de applicatie kunnen weergeven.
...
Kenmerken die enkel in een bepaalde browser ondersteund worden, worden voorafgaan door een browserprefix (zoals \code{-webkit-} voor WebKit gebaseerde browsers).
...
\st{} is net als Ext JS een \js{}-gedreven raamwerk met een Model-View-Controller~(MVC) ontwerppatroon.
\st{} biedt een commerciële licentie waarbij het bedrijf in kwestie de broncode niet publiek mag maken.  
Een GNU's Not Unix General Public License v3~(GNU GPLv3)-versie van \st{} laat dit wel toe.
Beide licenties zijn gratis.
Op het moment van schrijven is \st{} aan versie 2.2~\cite{Inc.}. 

\subsubsection{\tmp} % SANDER
...
Dit raamwerk is volledig \js{}-gedreven en steunt op het MVC-ontwerppatroon.
...
De GI-elementen bootsen de \term{native look-and-feel} van iOS na.

\subsubsection{\kendo} % SANDER
...
Het eerste luik is gericht op de ontwikkeling van desktop- en mobiele applicaties,  het tweede voegt \term{native look-and-feel} toe aan mobiele applicaties en het laatste zorgt voor datavisualisatie met HTML5 en \js{}.
...
Verder heeft de ontwikkelaar ook de mogelijkheid om een \term{backend} te integreren met de klantzijde.
...
\subsubsection{\moobile} % TIM
\moobile{} is een jong raamwerk dat op het moment van schrijven aan versie~0.2.1 zit sinds de eerste \term{commit} op \gh{} in 2011~\cite{Dery2013}.
...
Een bijzonderheid is dat de lay-out de \term{native look-and-feel} van Android of iOS nabootst.
...
Het raamwerk gebruikt een architectuur dat compatibel is met \term{open-source} bibliotheken zoals jQuery~\cite{JQuery2013a}, KnockOut~\cite{KnockOut2013} en Backbone~\cite{Backbone2013}.
...
De vijf categorieën die horen bij dit model worden hieronder geciteerd:
\begin{itemize}
...
Deze vergelijkt jQTouch,  \jqm{},  \st{},  PhoneGap en Titanium.  
...
jQTouch,  \jqm{} en PhoneGap kunnen hiertoe gerekend worden.
...
Op een blogpost van Codefessions wordt een vergelijking gemaakt tussen \jqm{}, \st{}, jQTouch en \kendo{}~\cite{Sarrafi2012a}.  
...
Het rapport over Cross-platform Developer Tools van Vision Mobile~\cite{Mobile2012} vergelijkt $15$ \term{cross-platform} tools.
...
Bij de evaluatie wordt bij elk criterium het gemiddelde van dat criterium voor alle andere \term{cross-platform} tools geplaatst.
...
Oehlman et al.~\cite{Oeflman2011} vergelijken Jo, jQTouch,  \jqm{} en \st{} door een geosociaal spel te ontwikkelen genaamd Moundz. 
