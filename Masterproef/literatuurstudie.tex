...
% TODO voor Sander: is deze afbeelding nuttig? 
% TODO afbeelding vectorieel maken
\begin{figure}
  \centering
...
2G gebruikte daarentegen een digitaal netwerk, waarbij nieuwe diensten zoals SMS tekstberichten beschikbaar werden.
...
De vraag naar breedband blijft, waardoor High-Speed Downlink Packet Access~(HSDPA) werd ontwikkeld. 
...
Dit maakt gebruik van LTE Advanced, een verbetering van Long Term Evolution~(LTE).

\paragraph{Draadloos Internet}
Net zoals laptops draadloos verbinden maken met Internet via Wifi, kan dit ook op mobiele apparaten.
...
\pgfplotstabletypeset[
  ...
  string type,
  skip coltypes=true,
  columns={BrowserNL,Marktaandeel},
  columns/BrowserNL/.style={column name=\textbf{Mobiele webbrowser}},  
  columns/Marktaandeel/.style={column name=\textbf{Marktaandeel (\%)}},
  every head row/.style={
    before row=\toprule,
    after row=\midrule},
...
%TODO Tim: introductie waarom dat nodig is
...
Hierdoor werden acht raamwerken geselecteerd waaruit uiteindelijk vier raamwerken gedetailleerd besproken en vergeleken zullen worden in respectievelijk hoofdstukken \ref{chap:raamwerken} en \ref{chap:vergelijkingscriteria}.
De motivatie hiervoor is te vinden in sectie~\ref{sec:motivatie-acht-raamwerken}.

% TODO Tim: enkel termen uitleggen
