...
% GUI designer
% Theme designer
% markadoptatie (belangrijke klanten)
% licenties/kost/betalende support

% %Code en development
% Wat is belangrijk met betrekking tot de programmeur:  moet er enkel JavaScript worden geschreven,  of ook HTML, … Zijn er mogelijkheden tot debugging?
% Wat is de kwaliteit van de code en hoe ziet de code eruit nadat het framework is toegepast.  
% Enterprise support

% %functionele kenmerken
% Een beschrijving van de belangrijkste mogelijkheden en hun gemak om ze te implementeren.  Enkele mogelijkheden
% listviews
% Forms/Buttons/Validation
% Grafieken
% Pagina (en hun layout) / Dialogs / Popups
% Touchevents
% Dataflow (in en uit)
% SAP interactie?

% %niet-functionele
% Een beschrijving van de niet-functionele features die het framework aanbiedt.  Enkele mogelijkheden zijn:
% 
% Performantie (snelheid van het framework)
% Aanpasbaasheid van themas (CSS)
% Programmeerbaarheid (hergebruik van onderdelen,  MVC, …)
% Ondersteuning voor browser en OS
...
Zoals werd aangehaald, schrijft men vooral HTML5 code voorzien van \code{data-}* attributen. 
Daarna zal het raamwerk door middel van \term{progressive enhancement} allerhande code toevoegen om de beoogde UI elementen correct te tonen in de browser. Dit wordt verder uitgelegd in de sectie browserondersteuning (zie \ref{sec:jqm-browser-support}).

Er zijn drie strategieën om webapplicaties te maken in jQuery Mobile~\cite{Broulik2012}. 
...
jQuery Mobile biedt alle gangbare elementen voor formulieren aan zoals textinvoer, een selectie uit een lijst, een zoekveld, een \term{slider} en een \term{switch}. 
Het raamwerk verplicht zelf om de \code{<label>}-tag te gebruiken. 
...
jQuery Mobile zal enkel die \code{<div>} verder aanvullen die op dat moment getoond wordt aan de gebruiker. 

\paragraph{Aanpasbaarheid}
Als je jQuery Mobile \term{out-of-the-box} gebruikt, zit alles al goed qua kleur en design. 
...
Hier is jQuery Mobile op voorzien door hun \term{stylesheet} op te delen in twee delen: thema's en structuur. 
...
Eenmaal tevreden kan je de overeenkomstige \term{stylesheet} downloaden en toevoegen aan je project.
...
Daarna, als de ontwikkelaar nog tijd heeft, gaat hij \term{fallbacks} implementeren waardoor minder recente browser de applicatie ook kunnen weergeven.
...
De broncode van Sencha Touch kan ook ingeladen worden met \code{sencha-touch-debug.js} als bibliotheek.  
...
Sencha thema's bestaan allemaal uit een set van mixins.  
Door zelf mixins te creëren of reeds bestaande te manipuleren kunnen we eigen thema's creëren en ze aan de \code{ui}-waarde van een component toekennen.
...
Sencha Touch steunt op de WebKit browser \term{engine} dus moet de browser deze bevatten.  
