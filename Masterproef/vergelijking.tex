...
%TODO productiviteit schrappen?
De implementatie van deze POC zal het productiviteits-, gebruiks- en ondersteuningscriterium drijven.  
%TODO gonzalo snapt onderstaande zin niet. Het wordt al vermeld bij gebruik. Hiet weglaten?
...
Een tabel zal voorzien worden met in de rijen het aantal volgers op Twitter, sterren en forkers van GitHub,  vragen op Stackoverflow en aantal vind-ik-leuks van Facebook~\cite{Sarrafi2012a,Ayuso2012}. 
...
Raamwerken die niet op GitHub te vinden zijn kruigen nul voor zowel het aantal sterren en forkers.
...
Zo kan éénzelfde persoon zowel een volger op Twitter zijn als een vind-ik-leuk op Facebook plaatsen.
...
Enerzijds kunnen veel vragen over één onderwerp duiden op veel onduidelijkheden over dit onderwerp.
...
Vervolgens zal de kwaliteit en kwantiteit van de documentatie van elk raamwerk worden bekekenen.
De mogelijkheden voor debuggen bepalen ook de productiviteit en zullen onderzocht worden.
Dan zal gekeken worden naar de aanwezige literatuur van het raamwerk en waar ontwikkelaars met vragen terecht kunnen.
Ten slotte zullen de lijnen code die nodig waren voor zowel de POC als de login applicatie gepresenteerd worden.
...
De POC kan onderverdeeld worden in verschillende deelproblemen.
...
In totaal konden $13$ uitdagingen en $38$ deeluitdagingen uit de POC geëxtraheerd worden.

De wijze waarop het raamwerk de uitdaging aangaat zal een score bepalen.
Er onderscheiden zich drie gevallen.
De hoogste score ($2$) wordt toegekend wanneer de functionaliteit reeds aangeboden wordt door het raamwerk. 
...
Omdat er $38$ deeluitdagingen zijn, kan het raamwerk voor dit criterium maximaal $76$ behalen.

%TODO alternatief voor ``interessante'' functionaliteit
Alle interessante functionaliteit die in de POC werd gebruikt, zit in een uitdaging vervat.  
Interessante functionaliteit slaat op functionaliteit die potentieel door het raamwerk kan worden geleverd.
...
De score van een uitdaging of deeluitdaging kan $1$ of $0$ zijn.
...
Deze worden in tabel \ref{tabel:toestellen-hci} weergegeven.
 ...
 %TODO getallen voluit schrijven?
...
\end{equation}
met $r$ de verschillende raamwerken,  $N_F$ het aantal geïmplementeerd uitdagingen en deeluitdagingen van het raamwerk,  $c$ de verschillende contexten en $i$ de uitdagingen. 
...
Indien de $8$ contexten alle uitdagingen en deeluitdagingen correct weergeven zal de maximale score van $104$ behaald worden.
...
Performantie kan worden opgesplitst in twee verschillende factoren: laadtijden en gebruikservaring.

Bij de laadtijden onderscheiden zich vier gevallen die samen de totale laadtijd bepalen.
Eerste zal de laadtijd van de volledig geïmplementeerde POC bekeken worden ($l_{r,c,POC}$). 
Vervolgens zal de tijd worden opgemeten om de POC uit het cachegeheugen te laden ($l_{r,c,POC_{cache}}$).
Dezelfde twee gevallen zullen ook voor de login applicatie die in sectie \ref{sec:vergelijking-productiviteit} werden geïntroduceerd, worden opgemeten ($l_{r,c,login}$ en $l_{r,c,login_{cache}}$).
De redenen waarom deze applicatie wordt gebruikt, is omdat er wordt verwacht dat niet de volledige POC in ieder raamwerk zal kunnen worden geïmplementeerd. 
Dit is in tegenstelling tot deze login applicatie wel met de vier raamwerken kan worden gebouwd.
...
Wireshark of TCPdump kunnen op de hotspot HTTP-verkeer opmerken en de HAR-bestanden aanmaken.
...
De gebruikservaring zal gemeten worden met de tijd die het raamwerk nodig heeft om GI-elementen te renderen.
Hiervoor zal de lijst van $850$ elementen worden gebruikt die getoond wordt na aanmelden op de login applicatie.
De tijd die het raamwerk nodig heeft om de lijst de renderen ($l_{r,c,lijst}$) kan gemeten worden met \js-code.
...
%Sander:  voor ge de lijst ziet moet toch eerst de applicatie laden en dan de lijst renderen.  De gebruiker moet op beide wachten..
...
Deze tool kan de code van een webapplicatie analyseren en de performantie testen specifiek voor mobiele apparaten.
