...
Deze applicatie toont de evolutie van zoektermen op Google op een schaal van 100. %TODO correcter
...
% TODO nog weg doen motivatie titel
\paragraph{Motivatie}
...
%TODO nog vermelden dat datepicker uitdagingen zijn samengenomen : JA
...
\begin{equation}
  O_r = \sum_{c=1}^{8}{\left(\sum_{i=1}^{N}U_{r,c,i}\right)}
...
%TODO
% 2 soorten performantie: het opstarten en het effectief renderen (duidelijk onderscheid uitschrijven)
% zeggen dat we in JS de snelheid van renderen van 850 lijstelementen zullen opmeten 
...
Bij de laadtijden onderscheiden zich vier gevallen.
...
De tijd die het raamwerk nodig heeft om de lijst de renderen ($r_{r,c,lijst}$) kan gemeten worden in \js.
...
\begin{equation}
  PE_r= \sum_{c=1}^{8}{\left(l_{r,c,POC}+l_{r,c,POC_{cache}}+l_{r,c,login}+l_{r,c,login_{cache} + r_{r,c,lijst}}\right)} 
...
De meeste tools die spinnenwebben genereren vereisen dit ook. %TODO checken (matlab plugin als referentie)
...
\begin{equation}
  REL(GM_r)=\frac{GM_r}{\max_{m}{GM_m}}
  \label{eq:rel-populariteit}
\end{equation}

\begin{equation}
  REL(PR_r) = \frac{\left(PR_R\right)^{-1}}{\max_{m}{\left(PR_m\right)^{-1}}}
  \label{eq:rel-productiviteit}
\end{equation}

\begin{equation}
  REL(GB_r) = \frac{GB_R}{\max_{m}{GB_m}}
  \label{eq:rel-gebruik}
\end{equation}

\begin{equation}
  REL(O_r) = \frac{O_r}{\max_{m}{O_m}}
  \label{eq:rel-ondersteuning}
\end{equation}

\begin{equation}
  REL(PE_r)= \frac{\left(PR_r\right)^{-1}}{\max_{m}{\left(PR_m\right)^{-1}}}
