...
De uitkomsten van dit criterium zullen worden vergeleken met data geleverd door Google Trends~\cite{Google2012a}.
...
%TODO bekijken evolutie van data in de tijd? ja
...
Elk zullen ze de POC in twee verschillende raamwerken maken en daarnaast ook een extra login applicatie in twee andere raamwerken.
De ene auteur maakt de POC in \jqm{} en \lungo{} en de loginapplicatie in \st{} en \kendo{}.
De andere zal dan de POC in \st{} en \kendo{} maken en de loginapplicaties in \jqm{} en \lungo{}.
...
\pgfplotstabletypeset[
  ...
  end table=\caption{Uitdagingen}\label{tabel:uitdagingen}\end{longtable},
  skip coltypes=true,
  col sep=comma,
  string type,
  header=true,
...
Performantie kan worden opgesplitst in twee verschillende factoren: laadtijden en rendertijden.
...
Bij de laadtijden onderscheiden zich twee gevallen die samen de totale laadtijd bepalen.
Eerste zal de laadtijd van de loginaaplicatie bekeken worden ($\widehat{l}_{r,c,login}$). 
Vervolgens zal de tijd worden opgemeten om de loginaaplicatie uit het cachegeheugen te laden ($\widehat{l}_{r,c,login_{cache}}$).

Het opmeten van laadtijden zal met PCAP Web Performance Analyzer gebeuren~\cite{SongL.bmcquadeMdsteele2010}.
Deze tool kan PCAP-bestanden die HTTP-trafiek bevatten, analyseren.
Om deze bestanden te verkrijgen zal een laptop als hotspot worden ingesteld en de apparaten geconnecteerd op deze hotspot.
TCPdump zal gebruikt worden op de hotspot om HTTP-verkeer op te meten en de PCAP-bestanden aan te maken.
...
De laadtijden en rendertijden zullen op acht verschillende apparaten worden opgemeten.
...
\begin{equation}
  \text{Totale laadtijd}_r= \sum_{c=1}^{8}{\left(\widehat{l}_{r,c,login}+\widehat{l}_{r,c,login_{cache}}\right)}
    \label{eq:totale-downloadtijd}
\end{equation}
\begin{equation}
  \text{Totale rendertijd}_r= \sum_{c=1}^{8}{\left(\widehat{l}_{r,c,lijst}\right)}
...
\begin{equation}
  \text{Performantie}_r= \frac{\text{Totale laadtijd}_r + \text{Totale rendertijd}_r}{8}
...
\begin{equation}
  \text{Maximale reponsetijd}_r= \frac{\sum_{c=1}^{8}\left(\widehat{l}_{r,c,login}\right) + \text{Totale rendertijd}_r}{8}
...
\begin{itemize}
\item $\text{Maximale reponsetijd}_r < 0.1$: de gebruiker heeft het gevoel dat het systeem direct reageert.
\item $\text{Maximale reponsetijd}_r < 1$: de gedachtengang van de gebruiker zal niet worden onderbroken, maar hij zal toch een vertraging waarnemen.
\item $\text{Maximale reponsetijd}_r < 10$: de limiet om de aandacht van de gebruiker te behouden.
\end{itemize}
...
\begin{equation}
  REL(\text{Productiviteit}_r) = \frac{\text{Productiviteit}_r^{-1}}{\max_{m}\{\text{Productiviteit}_m^{-1}\}}
...
\begin{equation}
  REL(\text{Performantie}_r)= \frac{\text{Performantie}_r^{-1}}{\max_{m}\{\text{Performantie}_m^{-1}\}}
